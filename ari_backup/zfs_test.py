...
class ZFSLVMBackupTest(test_lib.FlagSaverMixIn, unittest.TestCase):
  ...
  def testWorkflowRunsInCorrectOrder(
      self, mock_create_zfs_snapshot, mock_destroy_expired_zfs_snapshots):
    ...
    manager_mock.attach_mock(mock_create_zfs_snapshot, '_create_zfs_snapshot')
    manager_mock.attach_mock(
        mock_destroy_expired_zfs_snapshots, '_destroy_expired_zfs_snapshots')
    # Create mock.call objects and defined their expected call order.
    run_custom_workflow_call = mock.call._run_custom_workflow()
    create_zfs_snapshot_call = mock.call._create_zfs_snapshot(error_case=False)
    destroy_expired_zfs_snapshots_call = (
        mock.call._destroy_expired_zfs_snapshots(days=30, error_case=False))
  ...
  def testFindSnapshotsOlderThan_returnsOnlyPrefixedSnapshots(
    ...
    self.assertEqual(snapshots, ['zfs/homedirs@fake-prefix-2014-01-05--0630'])
